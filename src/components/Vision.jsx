...
const Vision = () => {
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 1, ease: "easeOut" }}
      className="min-h-screen flex items-center justify-center py-6 md:p-8 lg:p-12"
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 w-11/12 max-w-7xl">
        ...
        <div className="relative flex justify-center items-center">
          <motion.img
            src="/vv.webp"
            alt="Healthcare professionals"
            className="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-full h-auto object-cover rounded-3xl shadow-2xl border-4 border-white"
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ duration: 1, ease: "easeOut" }}
          />
